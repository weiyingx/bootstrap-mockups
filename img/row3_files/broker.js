...
//PRE BROKER REFERENCE FOR MS HP
if (/(en-us\/default\.aspx|en-us?$|en-us\/?$)/i.test(window.location)) {
var s = document.createElement("script");
s.src = "//i.s-microsoft.com/library/svy/pre_broker.js";
				...
				while (isMatch && i < content.length) {
					...
					if (c.element) {
						...
						for (var k = 0; k < elements.length; k++) {
							var val = new RegExp(c.elementValue,"i");
		...
		return {
			...
			*/
			...
			init: function() {
				...
				//tstMode section
				if (/cs\_tstMode/i.test(window.location)) {
					_utils.loadScript("https://www.microsoft.com/library/svy/" + _sr.testUrl, true);
				}
